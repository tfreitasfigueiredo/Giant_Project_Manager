...
export function RiskCard({ risk }: { risk: Risk }) {
  ...
  return (
    <Card className="border-slate-200">
      ...
      <CardContent className="grid gap-3 text-sm text-slate-600 md:grid-cols-3"><div><p className="font-medium text-slate-950">Impacto no go live</p><p>{risk.goLiveImpact}</p></div><div><p className="font-medium text-slate-950">Responsável</p><p>{risk.owner}</p></div><div><p className="font-medium text-slate-950">Mitigação</p><p>{risk.mitigation}</p></div></CardContent>
    </Card>
  );
}
