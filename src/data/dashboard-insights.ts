...
export const portfolioHealthRows = [
  ...
  { label: "Atenção", value: projects.filter((project) => project.status === "attention").length, tone: "orange", percent: 44 },
  { label: "Crítico", value: projects.filter((project) => project.status === "critical").length, tone: "red", percent: 28 },
...
export const riskRows = [
  ...
  { label: "Médio", value: risks.filter((risk) => risk.severity === "Médio").length, tone: "orange" },
  { label: "Baixo", value: 1, tone: "emerald" },
] as const;

export const issueRows = [
  { label: "Críticas abertas", value: issues.filter((issue) => issue.critical).length, tone: "red" },
  { label: "Altas abertas", value: 2, tone: "orange" },
  { label: "Médias abertas", value: 3, tone: "amber" },
